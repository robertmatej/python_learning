"""
English
Calculatong cos of travel
user give:
- car fuel consumption
- trip distance
- fuel type
- price of fuel if he want

By using Web scrapping actual fuel proces are automaticly downloaded from automotive website www.autocentrum.pl

Prices are presented in Polish Zloty
variables are named in polish language because of fast tralnslation :)

Web wrapping actual fuel prices are made only in educational purposes.
...
def licz_koszt(lbl_wynik333, get_trasa, get_spalanie):
    ...
    try:
        if reczna_cena:
            ...
            lbl_wynik.configure(
                text=f'Travel cost of {dlugosc_trasy}km on fuel price {cena}PLN is {round(koszt_przejazdu, 2)}PLN',
                font=35, fg='darkgreen', bg='lightgreen', wraplength=200)
        else:
            koszt_przejazdu = pobierz_cenę_paliwa()[rodzaj_paliwa] * float(spalanie) * float(dlugosc_trasy) * 0.01
            lbl_wynik.configure(
                text=f'Travel cost of {dlugosc_trasy}km on {print_paliwa[rodzaj_paliwa]} is {round(koszt_przejazdu, 2)}PLN',
                font=35, fg='darkgreen', bg='lightgreen', wraplength=200)
    except ValueError:
        lbl_wynik.configure(text=f'Please input proper parameters', font=80, bg='pink', fg='red')
    except KeyError:
        lbl_wynik.configure(text=f'Choose fuel type', font=80, fg='red')
    except TypeError:
        # pass
        lbl_wynik.configure(text=f'Price source acess problem, website not responding, enter fuel price manually',
                            font=80, fg='red', bg='pink', wraplength=200)
...
def pobierz_cenę_paliwa():
    ...
    # Błąd nie zostanie wyświetlony gdyż wcześniej zgłosi się błąd warości w słowniku, obsługa usuwa błąd połączenia w terminalu, przy pass też obsugiwało błąd
        ...
        lbl_wynik.configure(text=f'Fuel price source not respondign, chceck Internet connection', font=80, fg='red',
                            wraplength=200)
...
root = Tk()
root.title('Car travel cost calculator')
# root.geometry('450x550')


lbl_instruction = Label(text='Input car consumption', fg="green", font=30)
lbl_instruction.pack(fill=X, expand=True)

# pobranie spalania
ent_spalanie = Entry(text='Input average consumption', fg='blue', font=25, bg='lightgrey', width=25, justify=CENTER)
ent_spalanie.pack(padx=15, pady=5)
ent_spalanie.insert(0, '')

lbl_instr_trasa = Label(text='Input travel distance ', fg="green", font=30)
...
btn_paliwo_ON = Button(master=frm_paliwa, text='ON', relief='groove', bd=4, bg='black', fg='white', font=25, width=5,
                       height=2)
btn_paliwo_95 = Button(master=frm_paliwa, text='95', relief='groove', bd=4, bg='green', fg='white', font=25, width=5,
                       height=2)
btn_paliwo_98 = Button(master=frm_paliwa, text='98', relief='groove', bd=4, bg='green', fg='white', font=25, width=5,
                       height=2)
btn_paliwo_ONp = Button(master=frm_paliwa, text='ON+', relief='groove', bd=4, bg='black', fg='white', font=25, width=5,
                        height=2)
btn_paliwo_LPG = Button(master=frm_paliwa, text='LPG', relief='groove', bd=4, bg='blue', fg='white', font=25, width=5,
                        height=2)
...
# button ręcnzej ceny
btn_podaj_cene_paliwa = Button(master=frm_reczna_cena, text="Own fuel price", font=25, fg="orange", bg='purple', width=11,
                               height=2)
...
licz = Button(text="Calculate cost", font=25, fg='blue', bg='gold', width=13, height=2)
licz.pack(expand=True, pady=15)

lbl_wynik = Label(text='Travel cost PLN', font=35, fg='red', width=40, bg='lavender')
